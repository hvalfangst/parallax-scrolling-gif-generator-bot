...
pub fn start_gif_recording_loop(mut state: State, headless: bool, color_map: Vec<u8>, color_to_index_map: HashMap<u32, u8>) {
    ...
    loop {
        ...
        if should_process_frame(&last_update) {
            if frame_count < MAX_GIF_FRAMES {
                process_frame(state.scaled_buffer, &mut encoder, width, height, &mut frame_count, color_map.clone(), color_to_index_map.clone());
